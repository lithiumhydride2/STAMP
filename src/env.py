...
class Env:
    def __init__(
        ...
        self.graph_ctrl = GraphController(
            self.graph_size, self.start, self.k_size, self.obstacles
        )
    ...
    def reset(self, seed=None):
        ...
        self.d_to_target = np.linalg.norm(
            self.sample - self.underlying_distrib.mean, axis=1
        )
        self.route = []
    ...
    def step(self, next_node_index, global_step=0, eval_speed=None):
        ...
        alpha = (
            min(global_step // 1000 * 0.1, 1) if arg.curriculum else 1
        )  # 10k episodes
        d_len = np.linalg.norm(
        ...
        while remain_length > next_length:
            ...
            if not eval_speed:
                steplen = (
                    0.1 * sample_length * alpha * self.random_speed_factor
                )  # target speed at least 10x slower
            else:
            ...
            for idx in range(self.n_targets):
                if self.d_to_target[idx] < 0.1:  # FOV
                    ...
                    measure_value = 0.0
                self.gp_wrapper.GPs[idx].add_observed_point(
                    add_t(measure_coord.reshape(-1, 2), self.curr_t), measure_value
                )
    ...
    def plot(self, route, n, step, path, budget_list, rew_list, div_list):
        ...
        for i, gp in enumerate(self.gp_wrapper.GPs):
            y_pred = gp.plot(
                self.ground_truth,
                target_id=i,
                target_num=self.n_targets,
                target_loc=target_mean,
                all_pred=y_pred_sum,
                high_idx=self.high_info_idx,
                agent_loc=self.node_coords[self.current_node_index],
            )
            y_pred_sum.append(y_pred)
        # plt.scatter(self.start[:, 0], self.start[:, 1], c='r', s=15, zorder=10)
        points_display = [
            (self.graph_ctrl.find_point_from_node(path)) for path in route
        ]
        ...
        for target_div in range(div_list.shape[1]):  # chart
            plt.plot(
                ...
                div_list[:, target_div],
                alpha=0.5,
                c=target_cmap[target_div],
            )
        # plt.plot(budget_list, div_list.mean(axis=1), 'k--', alpha=0.7)
        plt.ylabel("JSDiv")
        plt.title(
            "{:g}/{:g} Reward:{:.3f}".format(
                self.budget_init - self.budget, self.budget_init, rew_list[-1]
            )
        )
        # ax2 = ax1.twinx()
        ...
        plt.tight_layout()
        plt.savefig(
            "{}/{}_{}_samples.png".format(path, n, step, self.graph_size), dpi=150
        )
